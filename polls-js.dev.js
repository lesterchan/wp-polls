...
// Helper to count the selected checkboxes and stick to the allowed maximum - adapted from https://roytuts.com/limit-number-of-checkbox-selections-using-javascript/
function poll_limit_checkbox_selection(current_checkbox) {
    if (current_checkbox.type != "checkbox") return false;
    let allCheckboxes = current_checkbox.closest('div.wp-polls-ans').querySelectorAll('input[type="checkbox"]');
    let limit = current_checkbox.closest('div.wp-polls-ans').dataset.ansmax;
    if (!allCheckboxes || !limit) return false;
    let checkedCount = 0;
    for (let i = 0; i < allCheckboxes.length; i++) {
        checkedCount += (allCheckboxes[i].checked) ? 1 : 0;
    }
