...
### Polls Table Name
global $wpdb;
$wpdb->questions = $wpdb->prefix . 'questions';
$wpdb->answers = $wpdb->prefix . 'answers';
$wpdb->votes = $wpdb->prefix . 'votes';
$wpdb->types = $wpdb->prefix . 'types';
...
function get_poll($temp_poll_id = 0, $display = true)
{
    ...
    if (intval(get_option('poll_currentpoll')) == -1) {
        ...
        switch ($temp_poll_id) {
            // Random Poll
            case -2:
                $poll_id = $wpdb->get_var("SELECT questions_id FROM $wpdb->questions WHERE questions_active = 1 ORDER BY RAND() LIMIT 1");
                break;
            // Latest Poll
            case 0:
                // Random Poll
                if (intval(get_option('poll_currentpoll')) == -2) {
                    $random_poll_id = $wpdb->get_var("SELECT questions_id FROM $wpdb->questions WHERE questions_active = 1 ORDER BY RAND() LIMIT 1");
    ...
    if ($pollresult_id == $poll_id) {
        ...
        $poll_active = $wpdb->get_var("SELECT questions_active FROM $wpdb->questions WHERE questions_id = $poll_id");
...
function poll_scripts_admin($hook_suffix)
{
    ...
    if (in_array($hook_suffix, $poll_admin_pages)) {
        ...
        wp_localize_script('wp-polls-admin', 'answersdminL10n', array(
...
function check_voted_ip($poll_id)
{
    ...
    $log_expiry_sql = '';
    if ($log_expiry > 0) {
        $log_expiry_sql = 'AND (' . current_time('timestamp') . '-(votes_timestamp+0)) < ' . $log_expiry;
    }
    // Check IP From IP Logging Database
    $get_voted_aids = $wpdb->get_col("SELECT votes_aid FROM $wpdb->votes WHERE votes_qid = $poll_id AND votes_ip = '" . get_ipaddress() . "' $log_expiry_sql");
    if ($get_voted_aids) {
        return $get_voted_aids;
    } else {
        return 0;
...
function check_voted_username($poll_id)
{
    ...
    $get_voted_aids = $wpdb->get_col("SELECT votes_aid FROM $wpdb->votes WHERE votes_qid = $poll_id AND votes_userid = $votes_userid $log_expiry_sql");
    if ($get_voted_aids) {
        return $get_voted_aids;
    } else {
        return 0;
...
function display_pollvote($poll_id, $display_loading = true)
{
    ...
    $poll_question = $wpdb->get_row("SELECT questions_id, questions_question, questions_totalvotes, questions_timestamp, questions_expiry, questions_multiple, questions_totalvoters FROM $wpdb->questions WHERE questions_id = $poll_id LIMIT 1");

    // Poll Question Variables
    $poll_question_text = stripslashes($poll_question->questions_question);
    $poll_question_id = intval($poll_question->questions_id);
    $poll_question_totalvotes = intval($poll_question->questions_totalvotes);
    $poll_question_totalvoters = intval($poll_question->questions_totalvoters);
    $poll_start_date = mysql2date(sprintf(__('%s @ %s', 'wp-polls'), get_option('date_format'), get_option('time_format')), gmdate('Y-m-d H:i:s', $poll_question->questions_timestamp));
    $poll_expiry = trim($poll_question->questions_expiry);
    ...
    $poll_multiple_ans = intval($poll_question->questions_multiple);
    ...
    $poll_answers = $wpdb->get_results("SELECT answers_aid, answers_qid, answers_answers, answers_votes FROM $wpdb->answers WHERE answers_qid = $poll_question_id ORDER BY " . get_option('poll_ans_sortby') . ' ' . get_option('poll_ans_sortorder'));
    ...
    if ($poll_question && $poll_answers) {
        ...
        foreach ($poll_answers as $poll_answer) {
            ...
            $poll_answer_id = intval($poll_answer->answers_aid);
            $poll_answer_text = stripslashes($poll_answer->answers_answers);
            $poll_answer_votes = intval($poll_answer->answers_votes);
...
function display_pollresult($poll_id, $user_voted = '', $display_loading = true)
{
    ...
    // Get Poll Question Data
    $poll_question = $wpdb->get_row("SELECT questions_id, questions_question, questions_totalvotes, questions_active, questions_timestamp, questions_expiry, questions_multiple, questions_totalvoters FROM $wpdb->questions WHERE questions_id = $poll_id LIMIT 1");
    ...
    $poll_question_totalvotes = intval($poll_question->questions_totalvotes);
    $poll_question_totalvoters = intval($poll_question->questions_totalvoters);
    $poll_question_active = intval($poll_question->questions_active);
    $poll_start_date = mysql2date(sprintf(__('%s @ %s', 'wp-polls'), get_option('date_format'), get_option('time_format')), gmdate('Y-m-d H:i:s', $poll_question->questions_timestamp));
    $poll_expiry = trim($poll_question->questions_expiry);
    if (empty($poll_expiry)) {
        $poll_end_date = __('No Expiry', 'wp-polls');
    ...
    // Get Poll Answers Data
    $poll_answers = $wpdb->get_results("SELECT answers_aid, answers_answers, answers_votes FROM $wpdb->answers WHERE answers_qid = $poll_question_id ORDER BY " . get_option('poll_ans_result_sortby') . ' ' . get_option('poll_ans_result_sortorder'));
    // If There Is Poll Question With Answers
    if ($poll_question && $poll_answers) {
...
if (!function_exists('get_poll_question')) {
    function get_poll_question($poll_id)
    {
        global $wpdb;
        $poll_id = intval($poll_id);
        $poll_question = $wpdb->get_var("SELECT questions_question FROM $wpdb->questions WHERE questions_id = $poll_id LIMIT 1");
...
if (!function_exists('get_questions')) {
    function get_questions($display = true)
    {
        global $wpdb;
        $totalquestions = intval($wpdb->get_var("SELECT COUNT(questions_id) FROM $wpdb->questions"));
        if ($display) {
            echo $totalquestions;
        } else {
...
if (!function_exists('get_answers')) {
    function get_answers($display = true)
    {
        global $wpdb;
        $totalanswers = intval($wpdb->get_var("SELECT COUNT(answers_aid) FROM $wpdb->answers"));
        if ($display) {
            echo $totalanswers;
        } else {
            return $totalanswers;
...
if (!function_exists('get_pollvotes')) {
    ...
    {
        global $wpdb;
        $totalvotes = intval($wpdb->get_var("SELECT SUM(questions_totalvotes) FROM $wpdb->questions"));
        if ($display) {
            echo $totalvotes;
        } else {
            return $totalvotes;
...
if (!function_exists('get_pollvoters')) {
    ...
    {
        global $wpdb;
        $totalvoters = intval($wpdb->get_var("SELECT SUM(questions_totalvoters) FROM $wpdb->questions"));
        if ($display) {
            echo $totalvoters;
        } else {
            return $totalvoters;
...
{
    $template_answers_archivelink = stripslashes(get_option('poll_template_answers_archivelink'));
    $template_answers_archivelink = str_replace("%POLL_ARCHIVE_URL%", get_option('poll_archive_url'), $template_answers_archivelink);
    if ($display) {
        echo $template_answers_archivelink;
    } else {
        return $template_answers_archivelink;
    }
}
...
function polls_archive()
{
    ...
    global $wpdb, $in_answers_archive;
    // Polls Variables
    $in_answers_archive = true;
    ...
    $poll_voted_aid = 0;
    $poll_id = 0;
    $answers_archive_output_archive = '';
    $polls_type = intval(get_option('poll_archive_displaypoll'));
    $polls_type_sql = '';
    // Determine What Type Of Polls To Show
    switch ($polls_type) {
        case 1:
            $polls_type_sql = 'questions_active = 0';
            break;
        case 2:
            $polls_type_sql = 'questions_active = 1';
            break;
        case 3:
            $polls_type_sql = 'questions_active IN (0,1)';
            break;
    }
    // Get Total Polls
    $total_polls = $wpdb->get_var("SELECT COUNT(questions_id) FROM $wpdb->questions WHERE $polls_type_sql AND questions_active != -1");
    ...
    // Get Poll Questions
    $questions = $wpdb->get_results("SELECT * FROM $wpdb->questions WHERE $polls_type_sql ORDER BY questions_id DESC LIMIT $offset, $polls_perpage");
    if ($questions) {
        foreach ($questions as $question) {
            $polls_questions[] = array('id' => intval($question->questions_id), 'question' => stripslashes($question->questions_question), 'timestamp' => $question->questions_timestamp, 'totalvotes' => intval($question->questions_totalvotes), 'start' => $question->questions_timestamp, 'end' => trim($question->questions_expiry), 'multiple' => intval($question->questions_multiple), 'totalvoters' => intval($question->questions_totalvoters));
            $poll_questions_ids .= intval($question->questions_id) . ', ';
        }
        $poll_questions_ids = substr($poll_questions_ids, 0, -2);
    }

    // Get Poll Answers
    $answers = $wpdb->get_results("SELECT answers_aid, answers_qid, answers_answers, answers_votes FROM $wpdb->answers WHERE answers_qid IN ($poll_questions_ids) ORDER BY " . get_option('poll_ans_result_sortby') . ' ' . get_option('poll_ans_result_sortorder'));
    if ($answers) {
        foreach ($answers as $answer) {
            $polls_answers[intval($answer->answers_qid)][] = array('aid' => intval($answer->answers_aid), 'qid' => intval($answer->answers_qid), 'answers' => stripslashes($answer->answers_answers), 'votes' => intval($answer->answers_votes));
        }
    }

    // Get Poll IPs
    $ips = $wpdb->get_results("SELECT votes_qid, votes_aid FROM $wpdb->votes WHERE votes_qid IN ($poll_questions_ids) AND votes_ip = '" . get_ipaddress() . "' ORDER BY votes_qid ASC");
    if ($ips) {
        foreach ($ips as $ip) {
            $polls_ips[intval($ip->votes_qid)][] = intval($ip->votes_aid);
        }
    }
    // Poll Archives
    $answers_archive_output_archive .= "<div class=\"wp-polls wp-polls-archive\">\n";
    foreach ($polls_questions as $polls_question) {
        ...
        // Archive Poll Header
        $template_archive_header = stripslashes(get_option('poll_template_answers_archiveheader'));
        // Poll Question Variables
        $template_question = stripslashes(get_option('poll_template_resultheader'));
        ...
        // Print Out Result Header Template
        $answers_archive_output_archive .= $template_archive_header;
        $answers_archive_output_archive .= $template_question;
        // Store The Percentage Of The Poll
        $poll_answer_percentage_array = array();
        foreach ($polls_answers[$polls_question['id']] as $polls_answer) {
            ...
            if (isset($polls_ips[$polls_question['id']]) && in_array($polls_answer['aid'], check_voted_multiple($polls_question['id'], $polls_ips[$polls_question['id']]))) {
                ...
                $template_answer = str_replace("%POLL_ANSWER_IMAGEWIDTH%", $poll_answer_imagewidth, $template_answer);
                // Print Out Results Body Template
                $answers_archive_output_archive .= $template_answer;
            } else {
                // Results Body Variables
                $template_answer = stripslashes(get_option('poll_template_resultbody'));
                $template_answer = str_replace("%POLL_ID%", $polls_question['id'], $template_answer);
        ...
        // Archive Poll Footer
        $template_archive_footer = stripslashes(get_option('poll_template_answers_archivefooter'));
        ...
        // Print Out Results Footer Template
        $answers_archive_output_archive .= $template_footer;
        // Print Out Archive Poll Footer Template
        $answers_archive_output_archive .= $template_archive_footer;
    }
    $answers_archive_output_archive .= "</div>\n";

    // Polls Archive Paging
    if ($max_page > 1) {
        $answers_archive_output_archive .= stripslashes(get_option('poll_template_answers_archivepagingheader'));
        if (function_exists('wp_pagenavi')) {
            $answers_archive_output_archive .= '<div class="wp-pagenavi">' . "\n";
        } else {
            $answers_archive_output_archive .= '<div class="wp-polls-paging">' . "\n";
        }
        $answers_archive_output_archive .= '<span class="pages">&#8201;' . sprintf(__('Page %s of %s', 'wp-polls'), number_format_i18n($page), number_format_i18n($max_page)) . '&#8201;</span>';
        if ($start_page >= 2 && $pages_to_show < $max_page) {
            $answers_archive_output_archive .= '<a href="' . polls_archive_link(1) . '" title="' . __('&laquo; First', 'wp-polls') . '">&#8201;' . __('&laquo; First', 'wp-polls') . '&#8201;</a>';
            $answers_archive_output_archive .= '<span class="extend">...</span>';
        }
        if ($page > 1) {
            $answers_archive_output_archive .= '<a href="' . polls_archive_link(($page - 1)) . '" title="' . __('&laquo;', 'wp-polls') . '">&#8201;' . __('&laquo;', 'wp-polls') . '&#8201;</a>';
        }
        for ($i = $start_page; $i <= $end_page; $i++) {
            if ($i == $page) {
                $answers_archive_output_archive .= '<span class="current">&#8201;' . number_format_i18n($i) . '&#8201;</span>';
            } else {
                $answers_archive_output_archive .= '<a href="' . polls_archive_link($i) . '" title="' . number_format_i18n($i) . '">&#8201;' . number_format_i18n($i) . '&#8201;</a>';
            }
        }
        if (empty($page) || ($page + 1) <= $max_page) {
            $answers_archive_output_archive .= '<a href="' . polls_archive_link(($page + 1)) . '" title="' . __('&raquo;', 'wp-polls') . '">&#8201;' . __('&raquo;', 'wp-polls') . '&#8201;</a>';
        }
        if ($end_page < $max_page) {
            $answers_archive_output_archive .= '<span class="extend">...</span>';
            $answers_archive_output_archive .= '<a href="' . polls_archive_link($max_page) . '" title="' . __('Last &raquo;', 'wp-polls') . '">&#8201;' . __('Last &raquo;', 'wp-polls') . '&#8201;</a>';
        }
        $answers_archive_output_archive .= '</div>';
        $answers_archive_output_archive .= stripslashes(get_option('poll_template_answers_archivepagingfooter'));
    }

    // Output Polls Archive Page
    return apply_filters('polls_archive', $answers_archive_output_archive);
}


// Edit Timestamp Options
function poll_timestamp($poll_timestamp, $fieldname = 'questions_timestamp', $display = 'block')
{
    global $month;
...
### Function: Check All Polls Status To Check If It Expires
add_action('polls_cron', 'cron_polls_status');
function cron_polls_status()
{
    global $wpdb;
    // Close Poll
    $close_polls = $wpdb->query("UPDATE $wpdb->questions SET questions_active = 0 WHERE questions_expiry < '" . current_time('timestamp') . "' AND questions_expiry != '' AND questions_active != 0");
    // Open Future Polls
    $active_polls = $wpdb->query("UPDATE $wpdb->questions SET questions_active = 1 WHERE questions_timestamp <= '" . current_time('timestamp') . "' AND questions_active = -1");
...
{
    global $wpdb;
    $poll_id = $wpdb->get_var("SELECT questions_id FROM $wpdb->questions WHERE questions_active = 1 ORDER BY questions_timestamp DESC LIMIT 1");
    return intval($poll_id);
}


### Check If In Poll Archive Page
function in_answers_archive()
{
        ...
        switch ($_REQUEST['view']) {
            ...
            case 'process':
                ...
                if ($poll_id > 0 && !empty($poll_aid_array) && check_allowtovote()) {
                    $is_poll_open = intval($wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $wpdb->questions WHERE questions_id = %d AND questions_active = 1", $poll_id)));
                    if ($is_poll_open > 0) {
                        $check_voted = check_voted($poll_id);
                        if ($check_voted == 0) {
                            if (!empty($user_identity)) {
                                $votes_user = htmlspecialchars(addslashes($user_identity));
                            } elseif (!empty($_COOKIE['comment_author_' . COOKIEHASH])) {
                                $votes_user = htmlspecialchars(addslashes($_COOKIE['comment_author_' . COOKIEHASH]));
                            } else {
                                $votes_user = __('Guest', 'wp-polls');
                            }
                            $votes_userid = intval($user_ID);
                            $votes_ip = get_ipaddress();
                            $votes_host = esc_attr(@gethostbyaddr($votes_ip));
                            $votes_timestamp = current_time('timestamp');
                            ...
                            if ($poll_logging_method == 1 || $poll_logging_method == 3) {
                                ...
                                $vote_cookie = setcookie('voted_' . $poll_id, $poll_aid, ($votes_timestamp + $cookie_expiry), apply_filters('wp_polls_cookiepath', SITECOOKIEPATH));
                            }
                            $i = 0;
                            foreach ($poll_aid_array as $answers_aid) {
                                $update_answers_votes = $wpdb->query("UPDATE $wpdb->answers SET answers_votes = (answers_votes+1) WHERE answers_qid = $poll_id AND answers_aid = $answers_aid");
                                if (!$update_answers_votes) {
                                    unset($poll_aid_array[$i]);
                                }
                                $i++;
                            }
                            $vote_q = $wpdb->query("UPDATE $wpdb->questions SET questions_totalvotes = (questions_totalvotes+" . sizeof($poll_aid_array) . "), questions_totalvoters = (questions_totalvoters+1) WHERE questions_id = $poll_id AND questions_active = 1");
                            if ($vote_q) {
                                foreach ($poll_aid_array as $answers_aid) {
                                    $wpdb->query("INSERT INTO $wpdb->votes VALUES (0, $poll_id, $answers_aid, '$votes_ip', '$votes_host', '$votes_timestamp', '$votes_user', $votes_userid)");
                                }
            ...
            switch ($_POST['do']) {
                ...
                case __('Delete All Logs', 'wp-polls'):
                    ...
                    if (trim($_POST['delete_logs_yes']) == 'yes') {
                        $delete_logs = $wpdb->query("DELETE FROM $wpdb->votes");
                        if ($delete_logs) {
                ...
                case __('Delete Logs For This Poll Only', 'wp-polls'):
                    ...
                    $questions_id = intval($_POST['questions_id']);
                    $questions_question = $wpdb->get_var("SELECT questions_question FROM $wpdb->questions WHERE questions_id = $questions_id");
                    if (trim($_POST['delete_logs_yes']) == 'yes') {
                        $delete_logs = $wpdb->query("DELETE FROM $wpdb->votes WHERE votes_qid = $questions_id");
                        if ($delete_logs) {
                            echo '<p style="color: green;">' . sprintf(__('All Logs For \'%s\' Has Been Deleted.', 'wp-polls'), stripslashes($questions_question)) . '</p>';
                        } else {
                            echo '<p style="color: red;">' . sprintf(__('An Error Has Occurred While Deleting All Logs For \'%s\'', 'wp-polls'), stripslashes($questions_question)) . '</p>';
                        }
                    }
                    break;
                ...
                case __('Delete Poll Answer', 'wp-polls'):
                    ...
                    $questions_id = intval($_POST['questions_id']);
                    $answers_aid = intval($_POST['answers_aid']);
                    $poll_answers = $wpdb->get_row("SELECT answers_votes, answers_answers FROM $wpdb->answers WHERE answers_aid = $answers_aid AND answers_qid = $questions_id");
                    $answers_votes = intval($poll_answers->answers_votes);
                    $answers_answers = stripslashes(trim($poll_answers->answers_answers));
                    $delete_answers_answers = $wpdb->query("DELETE FROM $wpdb->answers WHERE answers_aid = $answers_aid AND answers_qid = $questions_id");
                    $delete_votes = $wpdb->query("DELETE FROM $wpdb->votes WHERE votes_qid = $questions_id AND votes_aid = $answers_aid");
                    $update_questions_totalvotes = $wpdb->query("UPDATE $wpdb->questions SET questions_totalvotes = (questions_totalvotes-$answers_votes) WHERE questions_id = $questions_id");
                    if ($delete_answers_answers) {
                        echo '<p style="color: green;">' . sprintf(__('Poll Answer \'%s\' Deleted Successfully.', 'wp-polls'), $answers_answers) . '</p>';
                    } else {
                        echo '<p style="color: red;">' . sprintf(__('Error In Deleting Poll Answer \'%s\'.', 'wp-polls'), $answers_answers) . '</p>';
                    }
                    break;
                // Open Poll
                case __('Open Poll', 'wp-polls'):
                    ...
                    $questions_id = intval($_POST['questions_id']);
                    $questions_question = $wpdb->get_var("SELECT questions_question FROM $wpdb->questions WHERE questions_id = $questions_id");
                    $open_poll = $wpdb->query("UPDATE $wpdb->questions SET questions_active = 1 WHERE questions_id = $questions_id;");
                    if ($open_poll) {
                        echo '<p style="color: green;">' . sprintf(__('Poll \'%s\' Is Now Opened', 'wp-polls'), stripslashes($questions_question)) . '</p>';
                    } else {
                        echo '<p style="color: red;">' . sprintf(__('Error Opening Poll \'%s\'', 'wp-polls'), stripslashes($questions_question)) . '</p>';
                    }
                    break;
                // Close Poll
                case __('Close Poll', 'wp-polls'):
                    ...
                    $questions_id = intval($_POST['questions_id']);
                    $questions_question = $wpdb->get_var("SELECT questions_question FROM $wpdb->questions WHERE questions_id = $questions_id");
                    $close_poll = $wpdb->query("UPDATE $wpdb->questions SET questions_active = 0 WHERE questions_id = $questions_id;");
                    if ($close_poll) {
                        echo '<p style="color: green;">' . sprintf(__('Poll \'%s\' Is Now Closed', 'wp-polls'), stripslashes($questions_question)) . '</p>';
                    } else {
                        echo '<p style="color: red;">' . sprintf(__('Error Closing Poll \'%s\'', 'wp-polls'), stripslashes($questions_question)) . '</p>';
                    }
                    break;
                // Delete Poll
                case __('Delete Poll', 'wp-polls'):
                    ...
                    $questions_id = intval($_POST['questions_id']);
                    $questions_question = $wpdb->get_var("SELECT questions_question FROM $wpdb->questions WHERE questions_id = $questions_id");
                    $delete_poll_question = $wpdb->query("DELETE FROM $wpdb->questions WHERE questions_id = $questions_id");
                    $delete_poll_answers = $wpdb->query("DELETE FROM $wpdb->answers WHERE answers_qid = $questions_id");
                    $delete_poll_ip = $wpdb->query("DELETE FROM $wpdb->votes WHERE votes_qid = $questions_id");
                    $poll_option_lastestpoll = $wpdb->get_var("SELECT option_value FROM $wpdb->options WHERE option_name = 'poll_latestpoll'");
                    if (!$delete_poll_question) {
                        echo '<p style="color: red;">' . sprintf(__('Error In Deleting Poll \'%s\' Question', 'wp-polls'), stripslashes($questions_question)) . '</p>';
                    }
                    if (empty($text)) {
                        echo '<p style="color: green;">' . sprintf(__('Poll \'%s\' Deleted Successfully', 'wp-polls'), stripslashes($questions_question)) . '</p>';
                    }
...
function polls_page_general_stats($content)
{
    ...
    if ($stats_display['polls'] == 1) {
        ...
        $content .= '<ul>' . "\n";
        $content .= '<li>' . sprintf(_n('<strong>%s</strong> poll was created.', '<strong>%s</strong> polls were created.', get_questions(false), 'wp-polls'), number_format_i18n(get_questions(false))) . '</li>' . "\n";
        $content .= '<li>' . sprintf(_n('<strong>%s</strong> polls\' answer was given.', '<strong>%s</strong> polls\' answers were given.', get_answers(false), 'wp-polls'), number_format_i18n(get_answers(false))) . '</li>' . "\n";
...
class WP_Widget_Polls extends WP_Widget
{
    ...
    function widget($args, $instance)
    {
        ...
        $display_answers_archive = intval($instance['display_answers_archive']);
        if (isset($before_widget) && isset($after_widget) && isset($before_title) && isset($after_title)) {
            echo $before_widget;
            if (!empty($title)) {
                echo $before_title . $title . $after_title;
            }
            get_poll($poll_id);
            if ($display_answers_archive) {
    ...
    function update($new_instance, $old_instance)
    {
        ...
        $instance['display_answers_archive'] = intval($new_instance['display_answers_archive']);
        return $instance;
    }
    ...
    {
        global $wpdb;
        $instance = wp_parse_args((array)$instance, array('title' => __('Polls', 'wp-polls'), 'poll_id' => 0, 'display_answers_archive' => 1));
        $title = esc_attr($instance['title']);
        $poll_id = intval($instance['poll_id']);
        $display_answers_archive = intval($instance['display_answers_archive']);
        ?>
        <p>
            <label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', 'wp-polls'); ?> <input
        ...
        <p>
            <label
                for="<?php echo $this->get_field_id('display_answers_archive'); ?>"><?php _e('Display Polls Archive Link Below Poll?', 'wp-polls'); ?>
                <select name="<?php echo $this->get_field_name('display_answers_archive'); ?>"
                        id="<?php echo $this->get_field_id('display_answers_archive'); ?>" class="widefat">
                    <option
                        value="0"<?php selected(0, $display_answers_archive); ?>><?php _e('No', 'wp-polls'); ?></option>
                    <option
                        value="1"<?php selected(1, $display_answers_archive); ?>><?php _e('Yes', 'wp-polls'); ?></option>
                </select>
            </label>
        </p>
        <p>
            <label for="<?php echo $this->get_field_id('poll_id'); ?>"><?php _e('Poll To Display:', 'wp-polls'); ?>
                <select name="<?php echo $this->get_field_name('poll_id'); ?>"
                        id="<?php echo $this->get_field_id('poll_id'); ?>" class="widefat">
                    <option
                        value="-1"<?php selected(-1, $poll_id); ?>><?php _e('Do NOT Display Poll (Disable)', 'wp-polls'); ?></option>
                    <option
                        value="-2"<?php selected(-2, $poll_id); ?>><?php _e('Display Random Poll', 'wp-polls'); ?></option>
                    <option
                        value="0"<?php selected(0, $poll_id); ?>><?php _e('Display Latest Poll', 'wp-polls'); ?></option>
                    <optgroup>&nbsp;</optgroup>
                    <?php
                    $polls = $wpdb->get_results("SELECT questions_id, questions_question FROM $wpdb->questions ORDER BY questions_id DESC");
                    if ($polls) {
                        foreach ($polls as $poll) {
                            $questions_question = stripslashes($poll->questions_question);
                            $questions_id = intval($poll->questions_id);
                            if ($questions_id == $poll_id) {
                                echo "<option value=\"$questions_id\" selected=\"selected\">$questions_question</option>\n";
                            } else {
                                echo "<option value=\"$questions_id\">$questions_question</option>\n";
                            }
                        }
                    }
                    ?>
...
function polls_activate()
{
    ...
    $create_table = array();
    $create_table['questions'] = "CREATE TABLE $wpdb->questions (" .
        "questions_id int(10) NOT NULL auto_increment," .
        "questions_question varchar(200) character set utf8 NOT NULL default ''," .
        "questions_timestamp varchar(20) NOT NULL default ''," .
        "questions_totalvotes int(10) NOT NULL default '0'," .
        "questions_active tinyint(1) NOT NULL default '1'," .
        "questions_expiry varchar(20) NOT NULL default ''," .
        "questions_multiple tinyint(3) NOT NULL default '0'," .
        "questions_totalvoters int(10) NOT NULL default '0'," .
        "questions_type int(10) NOT NULL default '1'," .
        "PRIMARY KEY (questions_id)) $charset_collate;";
    $create_table['answers'] = "CREATE TABLE $wpdb->answers (" .
        "answers_aid int(10) NOT NULL auto_increment," .
        "answers_qid int(10) NOT NULL default '0'," .
        "answers_answers varchar(200) character set utf8 NOT NULL default ''," .
        "answers_votes int(10) NOT NULL default '0'," .
        "PRIMARY KEY (answers_aid)) $charset_collate;";
    $create_table['votes'] = "CREATE TABLE $wpdb->votes (" .
        "votes_id int(10) NOT NULL auto_increment," .
        "votes_qid varchar(10) NOT NULL default ''," .
        "votes_aid varchar(10) NOT NULL default ''," .
        "votes_ip varchar(100) NOT NULL default ''," .
        "votes_host VARCHAR(200) NOT NULL default ''," .
        "votes_timestamp varchar(20) NOT NULL default '0000-00-00 00:00:00'," .
        "votes_user tinytext NOT NULL," .
        "votes_userid int(10) NOT NULL default '0'," .
        "PRIMARY KEY (votes_id)," .
        "KEY votes_ip (votes_ip)," .
        "KEY votes_qid (votes_qid)," .
        "KEY votes_ip_qid (votes_ip, votes_qid)" .
        ") $charset_collate;";
    $create_table['types'] = "CREATE TABLE $wpdb->types (" .
        "types_id int(10) NOT NULL auto_increment," .
        "types_description varchar(10) NOT NULL default ''," .
        "PRIMARY KEY (types_id)) $charset_collate;";
    maybe_create_table($wpdb->questions, $create_table['questions']);
    maybe_create_table($wpdb->answers, $create_table['answers']);
    maybe_create_table($wpdb->votes, $create_table['votes']);
    maybe_create_table($wpdb->types, $create_table['types']);
    $wpdb->query("INSERT INTO $wpdb->types VALUES (1, 'DEFAULT');");
    $wpdb->query("INSERT INTO $wpdb->types VALUES (2, 'MULTIPLE');");
    $wpdb->query("INSERT INTO $wpdb->types VALUES (3, 'LIKES');");
    $wpdb->query("INSERT INTO $wpdb->types VALUES (4, 'SLIDER');");
    $wpdb->query("INSERT INTO $wpdb->types VALUES (5, 'STARS');");
    // Check Whether It is Install Or Upgrade
    $first_poll = $wpdb->get_var("SELECT questions_id FROM $wpdb->questions LIMIT 1");
    // If Install, Insert 1st Poll Question With 5 Poll Answers
    if (empty($first_poll)) {
        // Insert Poll Question (1 Record)
        $insert_questions = $wpdb->query("INSERT INTO $wpdb->questions VALUES (1, '" . __('How Is My Site?', 'wp-polls') . "', '" . current_time('timestamp') . "', 0, 1, '', 0, 0);");
        if ($insert_questions) {
            // Insert Poll Answers  (5 Records)
            $wpdb->query("INSERT INTO $wpdb->answers VALUES (1, 1, '" . __('Good', 'wp-polls') . "', 0);");
            $wpdb->query("INSERT INTO $wpdb->answers VALUES (2, 1, '" . __('Excellent', 'wp-polls') . "', 0);");
            $wpdb->query("INSERT INTO $wpdb->answers VALUES (3, 1, '" . __('Bad', 'wp-polls') . "', 0);");
            $wpdb->query("INSERT INTO $wpdb->answers VALUES (4, 1, '" . __('Can Be Improved', 'wp-polls') . "', 0);");
            $wpdb->query("INSERT INTO $wpdb->answers VALUES (5, 1, '" . __('No Comments', 'wp-polls') . "', 0);");
        }
    }
    ...
    add_option('poll_ans_sortby', 'answers_aid');
    add_option('poll_ans_sortorder', 'asc');
    add_option('poll_ans_result_sortby', 'answers_votes');
    add_option('poll_ans_result_sortorder', 'desc');
    // Database Upgrade For WP-Polls 2.1
    add_option('poll_logging_method', '3');
    add_option('poll_allowtovote', '1');
    maybe_add_column($wpdb->questions, 'questions_active', "ALTER TABLE $wpdb->questions ADD questions_active TINYINT( 1 ) NOT NULL DEFAULT '1';");
    // Database Upgrade For WP-Polls 2.12
    maybe_add_column($wpdb->votes, 'votes_userid', "ALTER TABLE $wpdb->votes ADD votes_userid INT( 10 ) NOT NULL DEFAULT '0';");
    add_option('poll_archive_url', site_url('answers_archive'));
    ...
    maybe_add_column($wpdb->questions, 'questions_expiry', "ALTER TABLE $wpdb->questions ADD questions_expiry varchar(20) NOT NULL default '';");
    add_option('poll_close', 1);
    // Database Upgrade For WP-Polls 2.20
    add_option('poll_ajax_style', array('loading' => 1, 'fading' => 1));
    add_option('poll_template_answers_archivelink', '<ul>' .
        '<li><a href="%POLL_ARCHIVE_URL%">' . __('Polls Archive', 'wp-polls') . '</a></li>' .
        '</ul>');
    add_option('poll_archive_displaypoll', 2);
    add_option('poll_template_answers_archiveheader', '');
    add_option('poll_template_answers_archivefooter', '<p>' . __('Start Date:', 'wp-polls') . ' %POLL_START_DATE%<br />' . __('End Date:', 'wp-polls') . ' %POLL_END_DATE%</p>');
    maybe_add_column($wpdb->questions, 'questions_multiple', "ALTER TABLE $wpdb->questions ADD questions_multiple TINYINT( 3 ) NOT NULL DEFAULT '0';");
    $questions_totalvoters = maybe_add_column($wpdb->questions, 'questions_totalvoters', "ALTER TABLE $wpdb->questions ADD questions_totalvoters INT( 10 ) NOT NULL DEFAULT '0';");
    if ($questions_totalvoters) {
        $questions_totalvoters = intval($wpdb->get_var("SELECT SUM(questions_totalvoters) FROM $wpdb->questions"));
        if ($questions_totalvoters == 0) {
            $wpdb->query("UPDATE $wpdb->questions SET questions_totalvoters = questions_totalvotes");
        }
    }
    // Database Upgrade For WP-Polls 2.30
    add_option('poll_cookielog_expiry', 0);
    add_option('poll_template_answers_archivepagingheader', '');
    add_option('poll_template_answers_archivepagingfooter', '');
    // Database Upgrade For WP-Polls 2.50
    delete_option('poll_archive_show');

    // Index
    $index = $wpdb->get_results("SHOW INDEX FROM $wpdb->votes;");
    $key_name = array();
    if (sizeof($index) > 0) {
        foreach ($index as $i) {
            $key_name[] = $i->Key_name;
    ...
    if (!in_array('votes_ip', $key_name)) {
        $wpdb->query("ALTER TABLE $wpdb->votes ADD INDEX votes_ip (votes_ip);");
    }
    if (!in_array('votes_qid', $key_name)) {
        $wpdb->query("ALTER TABLE $wpdb->votes ADD INDEX votes_qid (votes_qid);");
    }
    if (!in_array('votes_ip_qid_aid', $key_name)) {
        $wpdb->query("ALTER TABLE $wpdb->votes ADD INDEX votes_ip_qid_aid (votes_ip, votes_qid, votes_aid);");
    }
    // No longer needed index
    if (in_array('votes_ip_qid', $key_name)) {
        $wpdb->query("ALTER TABLE $wpdb->votes DROP INDEX votes_ip_qid;");
    }
