...
function vote_poll() {
    ...
    if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'polls')
    {
        ...
        switch($_REQUEST['view'])
        {
            // Poll Vote
            case 'process':
               function check_poll_errors($vote_q,$check_voted,$is_poll_open,$poll_id,$poll_aid_array,$is_real){
			if (!$vote_q) return 1;
                        if (!$check_voted == 0) return 2;
			if (!$is_poll_open > 0) return 3;
                        if(!$poll_id > 0 && empty($poll_aid_array) && !check_allowtovote()) return 4;
			if(!$is_real > 0) return 5;
			return 0;
		}
		...
		//gets errors
		$error_num = check_poll_errors($vote_q,$check_voted,$is_poll_open,$poll_id,$poll_aid_array,$is_real);
			...
			//checks for errors, and reports them to the user
			switch ($error_num) {
   				...
   				case 2:
				    ...
				    // if(!$is_real > 0)
        			    break;
    				default:
        			    printf(__('Error with Poll.', 'wp-polls'));
				    //error with errors.
			} 
                break;
            // Poll Result
