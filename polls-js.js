...
(function () {
	...
	var l10n = window.pollsL10n || {};
	var show_loading = parseInt(l10n.show_loading, 10) > 0;
	var show_fading = parseInt(l10n.show_fading, 10) > 0;
	...
	function poll_vote(current_poll_id) {
		var form = document.getElementById("polls_form_" + current_poll_id);
		if (!form) {
			return;
		}

		var multiple_field = document.getElementById(
			"poll_multiple_ans_" + current_poll_id
		);
		var poll_multiple_ans = multiple_field
			? parseInt(multiple_field.value, 10)
			: 0;

		var selected = [];
		var choices = form.querySelectorAll(
		...
		var poll_answer_id = selected.length
	...
	function poll_request(current_poll_id, view, extra_fields) {
		var nonce_field = document.getElementById(
	...
	function poll_process_success(current_poll_id, data) {
		var container = poll_container(current_poll_id);
		if (container) {
	...
	document.addEventListener("click", function (event) {
		...
		var trigger = target.closest("[data-poll-action]");
		if (!trigger) {
			return;
		}

		var poll_id = trigger.getAttribute("data-poll-id");
