...
class WP_Polls_Template {
	...
	public static function display_pollvote( $poll_id, $display = true ) {
		...
		if ( $poll_answers ) {
			foreach ( $poll_answers as $poll_answer ) {
				...
				$replace_vote_body = apply_filters( 'wp_polls_template_votebody_markup', $replace_vote_body, $poll_answer, array(
					'%POLL_ID%'                  => $poll_id,
	...
	public static function display_pollresult( $poll_id, $display = true ) {
		...
		if ( ! empty( $poll_answers_results ) ) {
			...
			if ( 'polla_votes' === $order_by ) {
				usort( $poll_answers_results, function( $a, $b ) use ( $sort_order ) {
					if ( 'desc' === $sort_order ) {
	...
	private static function archive_polls_question_with_result( $archive_limit, $page ) {
		...
		// Determine poll output
		$output = '';
		$i = 1;
		
		if ( $polls ) {
			foreach ( $polls as $poll ) {
				$output .= self::display_pollresult( $poll->pollq_id, false );
				$output .= '<br /><br />';
				$i++;
			}
		} else {
