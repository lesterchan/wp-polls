...
/**
 * Helper function to generate plugin URLs.
 *
 * @param string $path Relative path to the file.
 * @return string Full URL to the file.
 */
function poll_plugin_url( $path ) {
	return plugins_url( 'wp-polls/' . $path );
}
...
function poll_scripts() {
	if ( file_exists( get_stylesheet_directory() . '/polls-css.css' ) ) {
		...
		wp_enqueue_style( 'wp-polls', poll_plugin_url( 'polls-css.css' ), false, WP_POLLS_VERSION, 'all' );
	}
	if ( is_rtl() ) {
		if ( file_exists( get_stylesheet_directory() . '/polls-css-rtl.css' ) ) {
			...
			wp_enqueue_style( 'wp-polls-rtl', poll_plugin_url( 'polls-css-rtl.css' ), false, WP_POLLS_VERSION, 'all' );
		}
	}
	...
	wp_enqueue_script( 'wp-polls', poll_plugin_url( 'polls-js.js' ), array( 'jquery' ), WP_POLLS_VERSION, true );
	wp_localize_script(
		'wp-polls',
		'pollsL10n',
		array(
	...
	// Enqueue ranked choice polls specific files.
	wp_enqueue_style( 'wp-polls-ranked-choice', poll_plugin_url( 'polls-ranked-choice.css' ), array( 'wp-polls' ), WP_POLLS_VERSION, 'all' );
	wp_enqueue_script( 'wp-polls-ranked-choice', poll_plugin_url( 'polls-ranked-choice.js' ), array( 'jquery', 'wp-polls' ), WP_POLLS_VERSION, true );
}
...
function poll_tinymce_addplugin( $plugin_array ) {
	if ( WP_DEBUG ) {
		$plugin_array['polls'] = poll_plugin_url( 'tinymce/plugins/polls/plugin.js?v=' . WP_POLLS_VERSION );
	} else {
		$plugin_array['polls'] = poll_plugin_url( 'tinymce/plugins/polls/plugin.min.js?v=' . WP_POLLS_VERSION );
	}
	return $plugin_array;
}
